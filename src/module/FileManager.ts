...
export default class FileManager extends DFWModule {
    ...
    public async assingLocalFileAsync(filePath: string, options: UploadOptions = {}): Promise<dfw_file> {
        ...
        if (options.removeSameVariants && fileParent) {
     ...
     */
    public async purge() {
        let data: dfw_file[] = await this.instance.getModule(DatabaseManager).getModel("dfw_file").findAll({ where: { expire: { [Op.lt]: moment().toDate() } } }) as any;
